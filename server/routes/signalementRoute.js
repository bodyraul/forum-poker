...
router.delete("/signalementMessage/:id",auth,async(req,res)=>{
    try {
       ...
       await signalementASupp.deleteOne();
...
router.get("/nombreMessageSignaler",role,async(req,res)=>{
    try {
        ...
        const allMsgSignaler = await Message.find({signalement:true});
        const nbSignalement = allMsgSignaler.length;
        res.json(nbSignalement);
    } catch (error) {
        res.status(500).json(error.message);
    }
})
...
router.get("/afficherMsgSignaler/:id",role,async(req,res)=>{
    try {
        ...
        if(teste === false){
            return res.status(404).json("Ce message n'a pas été signalé.");
        }
